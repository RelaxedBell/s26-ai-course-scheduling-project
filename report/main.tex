...
\twocolumn[
  \begin{@twocolumnfalse}
    ...
    \begin{abstract}
      ...
      quality. Our final pipeline produces variable-credit, conflict-free
      schedules for representative student profiles.
    \end{abstract}
    \vspace{1.0em}
  \end{@twocolumnfalse}
]
...
conflict-free variable-credit schedules in under a few seconds of search, with
...
until graduation. Transcripts are supplied as a PDF upload from the front-end.
...
\draw[arrow] (transcript.east) -- (sentiment.west);
\draw[arrow] (catalog.east) -- (parser.west);
    ...
    \draw[arrow] (transcript.east) -- (bayes.west);
\draw[arrow] (catalog.east) -- (bayes.west);
  \end{tikzpicture}
...
\begin{itemize}[leftmargin=*,topsep=1pt,itemsep=1pt]
  ...
  \item \emph{Credit range.} The total credit count must lie in
    $[c_{\min}, c_{\max}]$; assignments outside this range are
    discarded.
\end{itemize}
...
with a 12--15 credit target, the full pipeline returns multiple conflict-free 
schedules with an average neural-net schedule score of \textbf{0.68}.
...
major-affinity feature. The intervention shifts the CS share from 14\,\%
to 43\,\% on average without sacrificing schedule scores -- in fact the
mean schedule score improves slightly, because the network had been
penalizing the unbalanced schedules it was producing before the constraint
solver's runtime cutoff.
...
\begin{table}[t]
  ...
  \begin{tabular}{lccc}
    \toprule
    & \textbf{CS share} & \textbf{Score} \\
    \midrule
    No bias term       & 0.14 & 0.61 \\
    With bias term     & \textbf{0.43} & \textbf{0.68} \\
    \bottomrule
  \end{tabular}
\end{table}
